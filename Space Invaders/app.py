...
class Invaders(GameApp):
    ...
    any lives across multiple waves). If you add new attributes, they need to be
    documented here.

    LIST MORE ATTRIBUTES (AND THEIR INVARIANTS) HERE IF NECESSARY
    _lives: Score message (GLabel)
    """
    ...
    def start(self):
        ...
        if self._state == STATE_INACTIVE:
            self._text = GLabel(text='Press "s" to play, p to pause during game', x= 400, y=350 ,font_size = 20)
        else:
            self._text = None
    ...
    def update(self,dt):
        ...
        """
        if (self.input.is_key_down('s') == True and self._state == STATE_INACTIVE):
        ...
        if self._state == STATE_ACTIVE:
            ...
            if self._text != None:
                self._text.text = 'Score: '+ str(self._wave._tp)
            if self._lives != None:
                self._lives.text = "Lives: " + str(self._wave._lives)
            if self.input.is_key_down('p') == True or (self._wave._ship == None and self._wave._lives < 3 and self._wave._lives !=0):
                self._state = STATE_PAUSED
            elif self._wave._lives == 0 or self._wave._d == -1 or self._wave._d == -2:
                self._state = STATE_COMPLETE
        if self._state == STATE_PAUSED:
            self._text = GLabel(text='Press "r" to resume', x= 400, y=350 ,font_size = 20)
        if (self.input.is_key_down('r') == True and self._state == STATE_PAUSED):
            self._state = STATE_CONTINUE
            self._text.x = 700
            self._text.y = 650
        if self._state == STATE_CONTINUE:
            self._wave._ship = Ship(x=GAME_WIDTH/2,y=SHIP_BOTTOM,
            width=SHIP_WIDTH,height=SHIP_HEIGHT,bottom=SHIP_BOTTOM,movement=SHIP_MOVEMENT,lives=self._wave._lives,source='ship.png')
            self._state = STATE_ACTIVE
        if self._state == STATE_COMPLETE:
            self.STATE_COMPLETE()
        if (self.input.is_key_down('a') == True and self._state == STATE_COMPLETE):
            self._text = None
            self._state = STATE_NEWWAVE
    ...
    def draw(self):
        ...
        if self._state == STATE_ACTIVE or self._state == STATE_CONTINUE:
    ...
    def STATE_COMPLETE(self):
        """
        This is what the game will be doing during state STATE_COMPLETE
        """
        if self._wave._d == -1:
            self._text = GLabel(text='You win! Press A to Play Again', x= 400, y=350 ,font_size = 20)
        elif self._wave._d == -2:
            self._text = GLabel(text='You lost! Your Score: '+ str(self._wave._tp) + '. Press A to Play Again', x= 400, y=350 ,font_size = 20)
        else:
            self._text = GLabel(text='You lost! Your Score: '+ str(self._wave._tp) + '. Press A to Play Again', x= 400, y=350 ,font_size = 20)
    ...
    def STATE_NEWWAVE(self):
        """
        This is what the game will be doing during state STATE_NEWWAVE
        """
        self._wave = Wave()
        self._text = GLabel(text='Score: '+ str(self._wave._tp) , x=700 , y=650 ,font_size = 20)
        self._lives = GLabel(text= 'Lives' + str(self._wave._lives), x= 100, y=650 ,font_size = 20)
