...
class Ship(GImage):
    ...
    """
    def __init__(self,x,y,width,height,bottom, movement, lives, source):
        super().__init__(x=x,y=y,width=width,height=height,bottom = bottom, movement = movement, lives = lives, source=source)
    ...
    def collides(self,bolt):
        ...
        """
        if bolt.getVelocity() < 0 and self.contains((bolt.x + BOLT_WIDTH/2, bolt.y + BOLT_HEIGHT/2)):
            return True
...
class Bolt(GRectangle):
    ...
    def __init__(self,x,y,width,height,fillcolor,velocity):
        super().__init__(x=x,y=y,width=width,height=height,fillcolor = 'black')
